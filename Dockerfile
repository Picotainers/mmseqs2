...
RUN apt-get update && \ 
apt-get install -y build-essential git zlib1g-dev gcc g++ binutils make autoconf automake cmake libz2-dev libatomic1 && \ upx && \
    apt-get clean && rm -rf /var/lib/apt/lists/*RUN git clone https://github.com/soedinglab/MMseqs2.git && \
   cd MMseqs2 && \
   mkdir build && \
   cd build && \
